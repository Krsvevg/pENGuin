...
class A1PSFirst : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_a1_psfirst)
        ...
        answerInput.addTextChangedListener(object : TextWatcher {
            override fun afterTextChanged(s: Editable?) {
                val isCorrectAnswer = s?.toString()?.equals("goes", ignoreCase = true) ?: false
                ...
                continueButton.setOnClickListener {
                    if (isCorrectAnswer) {
                        val intent = Intent(this@A1PSFirst, A1PSSecond::class.java)
